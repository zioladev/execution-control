...
import { allowAll, blockAll, indeterminateAll, unavailable } from './doubles.ts';
...
export interface ExecutionControlSubject {
  ...
  }): Promise<{ engagedControl: boolean }>;
}

export interface ConformanceCheck { name: string; pass: boolean; detail: string }
export interface ConformanceReport { pass: boolean; checks: ConformanceCheck[] }

const CANDIDATE = (i = 0): ExecutionCandidate => ({ provider: `p${i}`, tool: `tool_${i}`, arguments: { i }, effect: 'state-changing' });

// A metered wrapper over a control double: counts how many times evaluate() ran and records the order.
function meteredControl(inner: ExecutionControlProvider, order: string[]): { calls: () => number; provider: ExecutionControlProvider } {
  let calls = 0;
  return { calls: () => calls, provider: { async evaluate(c: ExecutionCandidate) { calls++; order.push('evaluate'); return inner.evaluate(c); } } };
}
 ...
 */
export async function runExecutionControlConformance(subject: ExecutionControlSubject): Promise<ConformanceReport> {
  ...
  async function scenario(effect: 'read' | 'state-changing', mode: 'off' | 'required', inner: ExecutionControlProvider | undefined, candidate = CANDIDATE()) {
    const order: string[] = [];
  ...
  for (const [name, dbl] of [['block', blockAll], ['indeterminate', indeterminateAll]] as const) {
  ...
  {
    let anyReached = 0;
    for (let i = 0; i < 3; i++) { const s = await scenario('state-changing', 'required', blockAll, CANDIDATE(i)); anyReached += s.reached; }
 ...
 * compare against.
 */
export const referenceSubject: ExecutionControlSubject = {
  async attempt({ effect, mode, control, candidate, reachProvider }) {
    ...
    if (!control) return { engagedControl: true };
    let disposition;
    try {
    ...
    if (disposition === 'allow') await reachProvider(); // only `allow` proceeds; block/indeterminate do not
